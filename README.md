...
* Core (*mysimbdp_coredms*): the core component is the most important to care when designing the platform. It works as PaaS (*Platform as a Service*) and the technology used depends on the data and its environmental particular characteristics. In our case we are working with tuples which stores rides on NYC taxies. There are a few aspects to take into account in order to select the best technology:
...
Explained all above, is the moment to choose a tecnology. In this case, MongoDB is a really good choice. Each tuple could be stored in a file. 
